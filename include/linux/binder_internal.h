...
struct binder_context {
	...
	std::mutex context_mgr_node_lock;
...
struct binder_stats {
	std::mutex br[_IOC_NR(BR_ONEWAY_SPAM_SUSPECT) + 1];
	std::mutex bc[_IOC_NR(BC_REPLY_SG) + 1];
	std::mutex obj_created[BINDER_STAT_COUNT];
	std::mutex obj_deleted[BINDER_STAT_COUNT];
};
